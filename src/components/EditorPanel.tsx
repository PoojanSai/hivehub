...
import {
  X, MessageSquare, GitBranch, Save, Globe, Users,
  FolderOpen, Plus, Zap, Terminal
} from 'lucide-react';
    ...
    <div className="welcome-view">
      ...
      <div className="welcome-content">
        ...
        <div className="welcome-hints">
          ...
          <div className="hint"><Zap size={11}/><span>3 collaborators online</span></div>
    ...
    <div className="editor-view">
      ...
      <div className="tab-bar">
        ...
        <div className="tab-bar-right">
          {collaborators.map(c => (
            <div key={c.id} className="tab-collab" style={{ background: c.color + '22', border: `1px solid ${c.color}44` }} title={c.name}>
              <span style={{ color: c.color, fontSize: '9px', fontWeight: 700 }}>{c.avatar}</span>
          ...
          <div className="code-scroll">
            {codeLines.map((line, i) => {
              ...
              return (
                <div
                  key={lineNum}
                  className={`code-line ${collab ? 'collab-line' : ''}`}
                  style={collab ? { background: collab.color + '12' } : {}}
                >
                  <span className="line-num">{lineNum}</span>
                  <span className="line-content">{highlight(line)}</span>
                  {collab && (
                    <span className="collab-label" style={{ background: collab.color + '33', color: collab.color, borderColor: collab.color + '66' }}>
                      {collab.name}
                    </span>
                  )}
                </div>
            ...
            <div className="chat-messages">
              {messages.map((msg, i) => (
                <div key={i} className="chat-msg">
                  <span className="msg-avatar" style={{ background: msg.color + '33', color: msg.color }}>{msg.userId === 'me' ? 'Y' : msg.color === '#a78bfa' ? 'A' : msg.color === '#34d399' ? 'R' : 'M'}</span>
      ...
      <div className="status-bar">
        ...
        <div className="status-right">
          ...
          <span className="status-item live"><Users size={10}/> {collaborators.length} live</span>
        </div>
      </div>
    </div>
  );
